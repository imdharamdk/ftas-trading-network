const { getAllTickerStats, getPrices } = require("./binanceService");
...
function findPointByAge(points = [], ageMs = 0) {
  if (!points.length) return null;
  const targetTs = points[points.length - 1].ts - ageMs;
...
function computePercentChange(current, previous) {
  if (!previous || !Number.isFinite(previous.price) || !Number.isFinite(current?.price) || previous.price <= 0) {
    return 0;
  }
  return Number((((current.price - previous.price) / previous.price) * 100).toFixed(3));
}

function getSignalLabel({ change30s, change2m, change5m, dayChange, nearHigh, nearLow }) {
  ...
  if (sameDirectionUp && change30s >= 0.18 && change2m >= 0.4 && nearHigh) {
    return "BREAKOUT_LONG";
  }
  if (sameDirectionDown && change30s <= -0.18 && change2m <= -0.4 && nearLow) {
  ...
  if (sameDirectionUp && change2m >= 0.2) {
    return "BULLISH";
  }
  if (sameDirectionDown && change2m <= -0.2) {
    return "BEARISH";
  }
  return "WATCH";
}
...
function buildSignalNote(signal, metrics) {
  const directionText = metrics.change2m >= 0 ? "up" : "down";
...
function buildTrendEntry(symbol, meta = {}, points = []) {
  ...
  const point30s = findPointByAge(points, 30_000);
  const point2m = findPointByAge(points, 2 * 60_000);
  const point5m = findPointByAge(points, 5 * 60_000);
  ...
  const change30s = computePercentChange(latest, point30s);
  const change2m = computePercentChange(latest, point2m);
  const change5m = computePercentChange(latest, point5m);
  const dayChange = Number(toNumber(meta.priceChangePercent).toFixed(2));
  const acceleration = Number((change30s - (change2m - change30s)).toFixed(3));
  const momentumScore = Number(
    (change30s * 2.8 + change2m * 1.7 + change5m * 1.15 + clamp(dayChange, -12, 12) * 0.18 + acceleration * 1.2).toFixed(2)
  );
  const signal = getSignalLabel({ change30s, change2m, change5m, dayChange, nearHigh, nearLow });
  const absoluteScore = Math.abs(momentumScore);
  const strength = absoluteScore >= 2.8 ? "STRONG" : absoluteScore >= 1.35 ? "MODERATE" : "EARLY";
...
async function refreshUniverse(nowMs = Date.now()) {
  ...
  const scored = tickers
    ...
    .map((ticker) => ({
      ...
      activityScore: computeActivityScore(ticker),
    }))
...
async function getLiveTrendScanner({ limit = 12 } = {}) {
  ...
  const entries = state.universe
    .map((symbol) => buildTrendEntry(symbol, state.metaBySymbol[symbol], state.historyBySymbol[symbol] || []))
    .filter(Boolean)
    .sort((left, right) => Math.abs(right.momentumScore) - Math.abs(left.momentumScore) || right.activityScore - left.activityScore)
  ...
  const summary = entries.reduce((accumulator, entry) => {
    ...
    return accumulator;
  }, { total: 0, bullish: 0, bearish: 0, breakouts: 0, reversals: 0, watch: 0 });
