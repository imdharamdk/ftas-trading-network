...
const sessionState = {
  ...
  refreshToken: null,
  expiresAt: 0,
  loginPromise: null, // FIX: parallel login calls block karne ke liye
};
...
async function login() {
  // Sirf ek hi login ek waqt par — baaki parallel calls wait karengi
...
async function getQuote({ exchange, symbolToken, tradingSymbol, symbolName }) {
  ...
  const data = await postSecure("/rest/secure/angelbroking/market/v1/quote/", {
    mode: "FULL", exchange, symboltoken: symbolToken, tradingsymbol: tradingSymbol, symbol: symbolName,
  });
...
async function getCandles({ exchange, symbolToken, interval, from, to }) {
  ...
  const data = await postSecure("/rest/secure/angelbroking/historical/v1/getCandleData", {
    exchange,
