  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        // Only 1m and 5m scalping signals in this section
        setActiveSignals(
          allActive.filter(s => s.source !== "SMART_ENGINE" && ["1m","5m"].includes(s.timeframe))
        );
        // Closed: show TP/SL hits AND expired (not hidden like before)
        setHistorySignals(
          allHistory.filter(s => s.source !== "SMART_ENGINE" && ["1m","5m"].includes(s.timeframe))
        );
      } catch (e) {
        if (mounted) setError(e.message);
      } finally {
    ...
    <AppShell subtitle="Binance Futures — Scalping signals (1m & 5m only)" title="Crypto Scalping">
        ...
        <section className="panel">
          <div className="panel-header">
            <div>
              ...
              <p style={{ fontSize: 12, color: "#64748b", margin: "4px 0 0" }}>
                Signals auto-expire: <strong>1m → 5 min</strong> &nbsp;|&nbsp; <strong>5m → 15 min</strong>
              </p>
            </div>
          ...
          {activeSignals.length > 0 ? (
            <div>
              {/* Expiry countdowns shown above table */}
              <div style={{ display:"flex", flexWrap:"wrap", gap:8, marginBottom:12 }}>
                {activeSignals.map(s => (
                  <div key={s.id} style={{ display:"flex", alignItems:"center", gap:6,
                    background:"rgba(255,255,255,0.04)", padding:"5px 10px", borderRadius:8, fontSize:12 }}>
                    <span className="signal-timeframe-badge">{s.timeframe}</span>
                    <span style={{ fontWeight:600, color:"#e2e8f0" }}>{s.coin?.replace("USDT","")}</span>
                    <span className={`pill ${s.side === "LONG" ? "pill-success" : "pill-danger"}`} style={{fontSize:10}}>{s.side}</span>
                    <ExpiryCountdown signal={s} />
                  </div>
              ...
              <SignalTable
                emptyLabel={loading ? "Loading signals..." : "No active scalp signals."}
                signals={activeSignals}
              />
            </div>
          ) : (
            <div style={{ textAlign:"center", padding:"40px 0", color:"#64748b" }}>
              {loading ? "⏳ Loading signals..." : "🔍 No active scalp signals right now. Engine is scanning..."}
            </div>
          )}
        </section>
      )}
      ...
      {tab === "closed" && (
        <section className="panel">
          ...
          <SignalTable
            compact
            emptyLabel="No closed scalp signals yet."
